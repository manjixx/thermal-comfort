...
def plt_summer():
    ...
    for i in range(len(names)):
        ...
        pmv_distribution.axes.set_title('{} \'s pmv distribution plot of 2021 summer'.format(str(i)), fontsize=10)
        pmv_distribution.set_xlabel('pmv', fontsize=10)
        pmv_distribution.set_ylabel('density', fontsize=10)
        plt.savefig('../result/summer/scatter_each_person/ {} \'s pmv distribution plot of 2021 summer'.format(str(i)), dpi=200,
                    bbox_inches='tight')
        plt.show()
...
def plt_winter():
    ...
    for i in range(len(names)):
        data = df.loc[df["no"] == i]
        pmv_distribution = sns.kdeplot(data['thermal sensation'], shade=True)
        pmv_distribution.axes.set_title('{} \'s pmv distribution plot of 2021 winter'.format(str(i)), fontsize=10)
        pmv_distribution.set_xlabel('pmv', fontsize=10)
        pmv_distribution.set_ylabel('density', fontsize=10)
        plt.savefig('../result/winter/scatter_each_person/{} \'s pmv distribution plot of 2021 winter'.format(str(i)), dpi=200,
                    bbox_inches='tight')
        plt.show()
